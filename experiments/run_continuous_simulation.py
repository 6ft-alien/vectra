...
from src.data.loader import load_engineered_dataset, load_raw_session
...
def run_simulation(
    ...
    # 1. Train verification model on background dataset
    print("\n[*] Training verification model on baseline dataset...")
    features_df, metadata_df = load_engineered_dataset()
    X = features_df.values
    y = metadata_df["is_illegal"].values.astype(int)

    # Use Random Forest as the high-accuracy verification backbone
    clf = get_baseline_models(random_state=RANDOM_STATE)["Random Forest"]
    clf.fit(X, y)
    print("[+] Verification model trained successfully.")

    # 2. Initialize Token Manager and Risk Engine
    ...
    # 3. Prepare telemetry stream (Authentic User -> Hijacker)
    # Find training session for legitimate user
    user_train_dir = Path("training_files") / target_user
    legit_files = list(user_train_dir.glob("session_*")) if user_train_dir.exists() else []

    # Find imposter session from test files or other user
    other_user = "user12" if target_user != "user12" else "user20"
    imposter_dir = Path("training_files") / other_user
    imposter_files = list(imposter_dir.glob("session_*")) if imposter_dir.exists() else []

    if not legit_files or not imposter_files:
        print("[!] Raw session files not found. Using feature-level dynamic synthesizer.")
        _simulate_synthetic(clf, token_mgr, session_token, hijack_at_sec, max_duration_sec)
        return

    # Load raw files
    df_legit = load_raw_session(legit_files[0])
    df_imposter = load_raw_session(imposter_files[0])

    print(f"[*] Loaded legitimate session stream: {len(df_legit)} events from {target_user}")
    print(f"[*] Loaded imposter attack stream: {len(df_imposter)} events from {other_user}")

    # Build hybrid timeline
    segmenter = SlidingWindowSegmenter(window_size_sec=10.0, stride_sec=2.0)

    legit_windows = segmenter.segment_session(df_legit)
    imposter_windows = segmenter.segment_session(df_imposter)

    # Create hybrid stream
    hybrid_timeline = []
    # Phase 1: Authentic user up to hijack_at_sec
    for start_t, end_t, vec in legit_windows:
        if end_t <= hijack_at_sec:
            hybrid_timeline.append((end_t, vec, False))

    # Phase 2: Imposter takes over workstation
    t_offset = hijack_at_sec
    for start_t, end_t, vec in imposter_windows:
        curr_sim_t = t_offset + end_t
        if curr_sim_t <= max_duration_sec:
            hybrid_timeline.append((curr_sim_t, vec, True))
    ...
    for sim_t, feat_vec, is_imposter in hybrid_timeline:
        ...
        prob = clf.predict_proba(feat_vec.reshape(1, -1))[0, 1]
        ...
        if action == PolicyAction.REVOKE_TOKEN and revocation_timestamp is None:
            ...
            print(f"Reason: Risk Score {risk_score:.3f} >= Tau_High (0.70). Session terminated.")
            print("!" * 85)
            break
...
def _simulate_synthetic(clf, token_mgr, session_token, hijack_at_sec, max_duration_sec):
    ...
    for t in timestamps:
        ...
        prob = np.random.uniform(0.75, 0.98) if is_imposter else np.random.uniform(0.05, 0.28)
